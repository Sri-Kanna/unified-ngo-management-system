...
router.post('/', authenticateJWT, authorize(['admin', 'staff']), async (req, res) => {
    ...
    try {
        ...
        await logActivity(req.user?.id, 'CREATE_DONATION', 'donations', newDonation.id, `Created donation of ₹${newDonation.amount}`);
...
router.put('/:id', authenticateJWT, authorize(['admin', 'staff']), async (req, res) => {
    ...
    try {
        ...
        await logActivity(req.user?.id, 'UPDATE_DONATION', 'donations', updatedDonation.id, `Updated donation of ₹${updatedDonation.amount}`);
...
router.delete('/:id', authenticateJWT, authorize(['admin']), async (req, res) => {
    try {
        ...
        await logActivity(req.user?.id, 'DELETE_DONATION', 'donations', req.params.id, `Deleted donation of ₹${deletedDonation.amount}`);

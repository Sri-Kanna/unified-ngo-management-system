...
router.post('/', authenticateJWT, async (req, res) => {
  try {
    ...
    const [benCount] = await db.select({ count: sql<number>`count(*)::int` }).from(schema.beneficiaries);
    const [volCount] = await db.select({ count: sql<number>`count(*)::int` }).from(schema.volunteers);
